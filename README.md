...
```
--output "OUTPUT"          sets the output filename (default: output.gbc)
--title "TITLE"            sets a custom menu title (default: 256M COLLECTION)
--split                    splits output files into 8 MB parts (default: False)
--toc-sort {index,offset}  sets what the table of contents is ordered by (default: index)
--no-wait                  don’t wait for user input when finished (default: False)
```
